...
public:
 uint16_t name;//Name of this Menu
 boolean (*canEnter)(LCDMenu&);//Function to be called when this menu is 'used'

 LCDMenu(uint16_t n);//Constructs the Menu with a name and a NULL use function (be careful calling it)
 LCDMenu(uint16_t n,boolean (*c)(LCDMenu&));//Constructs the Menu with a specified use function
 ...
 LCDMenu * getChild(uint16_t which);//Returns a pointer to the which'th child of this Menu
 LCDMenu * getSibling(uint16_t howfar);//Returns a pointer to the sibling howfar siblings away from this Menu
